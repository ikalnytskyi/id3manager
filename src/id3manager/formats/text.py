...
def _parse_picture_type(value: str) -> t.Tuple[str, id3.PictureType]:
    ...
    if space < 0:
        url = value
        type_name_or_value = None
    else:
        url = value[:space]
        type_name_or_value = value[space + 1 :] or None

    return url, utils.get_apic_picture_type(type_name_or_value)


def _parse_data_url(path: str) -> (str, t.Optional[str]):
    ...
    semicolon = metadata.find(";")
    if semicolon < 0:
        encoding = None
        mime_type = metadata or None
    else:
        encoding = metadata[semicolon + 1 :]
        mime_type = metadata[:semicolon] or None
